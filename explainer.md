...
It is worth noting that even with top-level partitioning restrictions, this scheme would still allow tracking by sufficiently motivated sites. For example, it would be possible for a central AdTech vendor (call them adtech.example.com) to implement OpenID Connect (or a similar protocol) as follows: A user visits https://medicalsite.example.org/info-about-embarrasing-disease. The site is a customer of adtech.example.com, so when it sees it doesn't have a session with an adtech JWT in it, it issues a 302 redirect to https://adtech.example.com/device-authorize?scope=devicetoken&response_type=code&client_id=medicalsite&redirect_uri=https%3A%2F%2Fmedicalsite.example.org%2Finfo-about-embarrasing-disease. Then, `adtech.example.com` is in a Secure Context and in its own top-level partition (not that of medicalsite.example.org), so can call the attestation API to get a token that uniquely identifies the devices. It then stores, on the server side, a link between a newly generated code and the unique identifier of the device, and 302 redirects the user immediately back to https://medicalsite.example.org/info-about-embarrasing-disease?code=6fghsdagkfs35633. On the server-side, medicalsite.example.org receives the code, and calls an endpoint on https://adtech.example.com to exchange the code for a JWT containing the unique identifier of the device. Then, the user decides to navigate to https://newssite.example.com, who is also a customer of adtech.example.com. newssite.example.com sees it doesn't have a session with an adtech JWT in it, it issues a 302 redirect to https://adtech.example.com/device-authorize?scope=devicetoken&response_type=code&client_id=newssite&redirect_uri=https%3A%2F%2Fnewssite.example.org%2F. As before, adtech identifies the device with the attestation API - and notably receives the same token since it is requested from the same top-level site. As before, it issues a code, and redirects, and newssite.example.org exchanges the code for the JWT. This code contains the same tracking token as was served to medicalsite.example.org, and medicalsite.example.org and newssite.example.org can exchange, through adtech or otherwise, information about that user. This type of approach is possible even with cookies, but in practice, tracking prevention software in the browser can mitigate it, in a way that is not observable to the newssite, adtech, or medicalsite, through techniques such as not persisting cookies for adtech.example.org. If widespread requirements for this scheme were implemented as a condition of access, it would be difficult for browsers and addons to offer a tracker-free experience (compared to the status quo) - the price of using such an addon would be that a significant percentage of the web is broken. To avoid this outcome, implementing user agents SHOULD request consent from the user individually for each site before allowing usage of the attestation API, and SHOULD warn the user of the privacy risks of allowing usage.
